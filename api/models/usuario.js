...
const UsuarioSchema = new mongoose.Schema({
    nome: {
        type: String,
        required: [true,"nome não pode ficar vazio."]
    },  
    email: {
        type: String,
        lowercase: true,
        unique: true,
        required: [ true, "e_mail não pode ficar vazio." ],
        index: true,
        match: [/\S+@\S+\.\S+/,  'e_mail é inválido.']
    },
    loja: {
        type: Schema.Types.ObjectId,
        ref: "Loja",
        required: [true,"Loja não pode ficar vazio."]
    },
    permissao: {
        type: Array,
        default: ["cliente"]
...
UsuarioSchema.plugin(uniqueValidator, { message: "Usuarios - já está sendo utiliza"});
