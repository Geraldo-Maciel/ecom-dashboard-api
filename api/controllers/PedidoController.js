...
class PedidoController {
    ...
    async store(req,res,next){
        ...
        try {   
            ...
            // CHECAR DADOS DE ENTREGA
            if(!EntregaValidation.checarValorPrazo(Cliente.endereco.CEP, Carrinho, entrega)) return res.status(422).send({ error: "Dados de Entrega Inválidos" });
...
//            if(!PagamentoValidation( Carrinho, pagamento )) return res.status(422).send({ error: "Dados de Pagamento Inválidos" });
            ...
            EmailController.enviarNovoPedido({ pedido, usuario: cliente.usuario });
            const administradores = await Usuario.find({ permissão: "admin", loja });
            administradores.forEach((usuario) => {
                EmailController.enviarNovoPedido({ pedido, usuario });
            });

            // Notificar via email = cliente e admin = novo pedido
            return res.send({ pedido: Object.assign({}, pedido._doc, {  entrega: novaEntrega, pagamento: novoPagamento, clinte }) });
//            return res.send({ pedido: Object.assign({}, pedido . _doc ,  {  entrega : novaEntrega ,  pagamento : novoPagamento , cliente } )  } ) ;
        }catch(e){
            next(e);
        }
    ...
    async remove(req,res,next){
        try {
            ...
            const administradores = await Usuario.find({ permissão: "admin", loja: pedido.loja });
            administradores.forEach((usuario) => {
                EmailController.cancelarPedido({ pedido, usuario });
//                EmailController.enviarNovoPedido({ pedido, usuario });
            });
