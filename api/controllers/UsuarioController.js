...
/*
const mongoose = require("mongoose");
//const Usuario = mangusto.model("Usuario");
const Usuario = mongoose.model("Usuario");
const enviarEmailRecovery = require("../helpers/email-recovery");
...
class UsuarioController {
    ...
    show(req, res, next){
        Usuario.findById(req.params.id).populate({ path: "loja" })
        .then(usuario => {
            if(!usuario) return res.status(401).json({ errors: "usuario-id não registrado"});
            return res.json({ 
                usuario: {
//                    id: usuario._id,
                    nome: usuario.nome,
...
//        });
    ...
    login(req, res, next){
        const { email, password } = req.body;
        Usuario.findOne({ email }).then((usuario) => {
            if(!usuario) return res.status(401).json({ errors: "Usuário-login não registrado"});
            if(!usuario.validarSenha(password)) return res.status(401).json({ errors: "Senha-login inválida"});
            return res.json({ usuario: usuario.enviarAuthJSON() });
        }).catch(next);
//          }).catch((err) => {
//                console.log(err);
...
//                });
    ...
    showCompleteRecovery(req, res, next){
        ...
        Usuario.findOne({ "recovery.token": req.query.token }).then(usuario => {
            if(!usuario) return res.render("recovery", { error: "não existe usuário com este token", success: null });
            if( new Date(usuario.recovery.date) < new Date() ) return res.render("recovery", { error: "Token expirado. Temnte novamente.", success: null });
            return res.render("recovery/store", { error: null, success: null, token: req.query.token });
        }).catch(next);
    }
