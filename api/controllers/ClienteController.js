...
/*
    async  removeAdmin ( req , res , next ) {
        tente  {
            const  cliente  =  aguarde  Cliente . findById ( req . params . id ) . populate ( "usuario" ) ;
            if ( ! cliente )  return  res . status ( 400 ) . enviar ( {  erro : "Cliente nao encontrado."  } )
            aguarde o  cliente . usuario . remove ( ) ;
            aguarde o  cliente . remove ( ) ;
            retornar  res . enviar ( {  deletado : verdadeiro  } ) ;
        } catch ( e ) {
            próximo ( e ) ;
        }
    }
