...
module.exports = ({ usuario, recovery }, cb) => {
    const  message  =  `
        ...
        <a href=" ${ link} /v1/api/usuarios/senha-recuperada?token= ${ recuperação. símbolo} ">
            $ { link } / v1 / api / usuarios / senha-recuperada? token = $ { recovery . token }
        </a>
        <br /> <br /> <hr />
        <p>
            Obs .: Se você não solicitou uma redefinição, apenas ignore esse email.
        </p>
        <br />
        <p> Atenciosamente, Loja TI </p>
    ` ;
    ...
    const  opcoesEmail  =  {
        de : "naoresponder@lojati.com" ,
        para : usuario . email ,
        assunto : "Redefinição de Senha - Loja TI" ,
        html : mensagem
    } ;

    if (  processo . env . NODE_ENV  ===  "production"  ) {
        transportador . sendMail ( opcoesEmail ,  ( erro ,  informações )  =>  {
            if ( erro ) {
                console . log ( erro ) ;
                return  cb ( "Aconteceu um erro no envio do email, tente novamente." ) ;
            }  else  {
